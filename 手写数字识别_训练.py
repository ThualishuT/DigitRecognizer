...
if __name__ == '__main__':
    ...
    with open("数据集/train.csv", 'r') as training_data_file:
        epochs_1 = 5  # 第一次训练世代数
    ...
    score_card = []  # 创建一个积分卡，用于记录神经网络在测试中的表现。

    with open("数据集/test.csv", 'r') as test_data_file:
        for record in test_data_file:
            values_list = record.split(',')

            correct_label = int(values_list[0])  # 每条记录的第一项为正确答案
            inputs = numpy.asfarray(values_list[1:]) / 255.0 * 0.99 + 0.01  # 格式化输入值
    ...
    # 统计分数，计算识别率。
    score_card_array = numpy.array(score_card)
    performance = score_card_array.sum() / score_card_array.size

    # 生成报告
    report = f"""识别率：{performance}
隐藏层节点数：{hidden_nodes}
第一次训练学习率：{learning_rate}
第一次训练世代数：{epochs_1}
第二次训练学习率：{n.lr}
第二次训练世代数：{epochs_2}
"""
    ...
    # 记录报告
    with open('测试数据.txt', 'a') as test_reports:
        test_reports.write(report + '\n')
